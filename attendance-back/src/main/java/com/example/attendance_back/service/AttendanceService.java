...
public class AttendanceService {
     ...
     * 勤怠情報を登録または更新を行うメソッド
     *
     * @param registerAttendanceDtoList 複数の勤怠情報を含むリスト
     */
    public void register(List<RegisterAttendanceDto> registerAttendanceDtoList) {
        // 現在ログインしているユーザー情報を取得
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String email = authentication.getName();
        // メールアドレスから従業員IDを取得
        ...
        for (RegisterAttendanceDto registerAttendanceDto : registerAttendanceDtoList) {
            // 年月日のパラメータを取得
